...
interface FiscPhaseNotificationProps {
  isOpen: boolean;
  action: GameAction;
  gameState: GameState;
  myPlayerId: string;
  onCallBluff: (fiscPlayerId: string) => void;
  onPass: () => void;
}

export function FiscPhaseNotification({
  isOpen,
  action,
  gameState,
  myPlayerId,
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-6">
        <div className="bg-[#16213e] border-2 border-[#e94560] rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
          ...
          <p className="text-[#a0a0a0]">
            Waiting for other players to decide on the Fisc claims...
          </p>
        </div>
      </div>
    );
  ...
  return (
    <Modal isOpen={isOpen} onClose={() => {}} title="Fisc Claims">
      <div className="text-center">
        <p className="text-lg text-[#a0a0a0] mb-4">
          The following players are taking 1 coin from you as Fisc:
        </p>

        <div className="space-y-3 mb-6">
          {fiscResponses.map((resp) => {
